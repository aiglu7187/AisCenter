...
function search(){
    ...
    var sotrId = document.getElementById("sotrId");
    if (sotrId != null){
        ...
        if (priyoms != null){  // если список сотрудников
            ...
            if (priyoms.childNodes.length > 0){                  
                for (loop = 0; loop < priyoms.childNodes.length; loop++){
                    ...
                    appendPriyom(id.childNodes[0].nodeValue, date.childNodes[0].nodeValue, usl.childNodes[0].nodeValue, reg.childNodes[0].nodeValue);
                }
...
function appendPriyom(id, date, usl, reg){
    var tab = document.getElementById("priyomTable");
    var tr = document.createElement("tr");
    var tdId = document.createElement("td");
    tdId.appendChild(document.createTextNode(id));
