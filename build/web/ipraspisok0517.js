...
function init() {
    ...
    var url = "ipraspisok?action=searchipra&result=search&fam=&name=&patr=&reg=&npr=&dpr=&sort=" + sort;
    requ(url, "GET");
...
function initArchive() {
    var sort = document.getElementById("sort").value;
    var url = "ipraspisok?action=searcharchive&result=search&fam=&name=&patr=&reg=&npr=&dpr=&sort=" + sort;
    requ(url, "GET");
    document.getElementById("sortascdo").onclick = sortIpraArch;
    document.getElementById("sortdescdo").onclick = sortIpraArch;
    document.getElementById("sortascomsu").onclick = sortIpraArch;
    document.getElementById("sortdescomsu").onclick = sortIpraArch;
    document.getElementById("sortascdo").style = "border : 1px solid";
}
...
function searchIpra() {
    ...
    var sort = document.getElementById("sort").value;
    var url = "ipraspisok?action=searchipra&result=search&fam=" + fam + "&name=" + name
            + "&patr=" + patr + "&reg=" + reg + "&npr=" + npr + "&dpr=" + dpr + "&sort=" + sort;
    requ(url, "GET");
    clearTable();
}
...
function searchIpraArchive() {
    ...
    var dpr = document.getElementById("searchDPr").value;
    var sort = document.getElementById("sort").value;
    var url = "ipraspisok?action=searcharchive&result=search&fam=" + fam + "&name=" + name
            + "&patr=" + patr + "&reg=" + reg + "&npr=" + npr + "&dpr=" + dpr + "&sort=" + sort;
    requ(url, "GET");
    clearTable();
}
        ...
        if (ipras != null) {
            if (ipras.childNodes.length > 0) {
                for (loop = 0; loop < ipras.childNodes.length; loop++) {
                    ...
                    var archive = ipra.getElementsByTagName("archive")[0];
                    if (archive == null) {
                        ...
                        appendArchive(id.childNodes[0].nodeValue, fam.childNodes[0].nodeValue, name.childNodes[0].nodeValue,
                                patr.childNodes[0].nodeValue, dr.childNodes[0].nodeValue, region.childNodes[0].nodeValue,
                                prikazn.childNodes[0].nodeValue, prikaz.childNodes[0].nodeValue, omsudate.childNodes[0].nodeValue,
                                otchomsu.childNodes[0].nodeValue, otchcenter.childNodes[0].nodeValue, otchdo.childNodes[0].nodeValue);
                    }
                }
            }
        }
...
function appendArchive(id, fam, name, patr, dr, region, prikazn, prikaz, omsudate, otchomsu, otchcenter, otchdo) {
    var tbody = document.getElementById("tabbody");
    if (tbody != null) {
        var tr = document.createElement("tr");
        var tdId = document.createElement("td");
...
function openIpra() {
    ...
    var url = "ipraspisok?action=open&id=" + id;
    window.open(url, '_blank');
}

function printRed() {
    var url = "print?type=iprared";
    window.open(url, '_blank');
}

function uploadReestr() {
    ...
    if (fileName == "") {
        ...
        var url = "ipraupload?filename=" + fileName + "&id=upload";
        window.open(url);
    }
}