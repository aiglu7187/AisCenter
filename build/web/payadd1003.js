...
function parseMessages(responseXML) {   // преобразование полученного xml
    if (responseXML == null) {
        ...
        // родители
        var parents = responseXML.getElementsByTagName("parents")[0];
        // педагоги
        var peds = responseXML.getElementsByTagName("peds")[0];
        // основные статусы
        ...
        if (param != null) {
            ...
            if (parents.childNodes.length > 0) {
                for (loop = 0; loop < parents.childNodes.length; loop++) {
                    var parent = parents.childNodes[loop];
                    var pId = parent.getElementsByTagName("id")[0];
                    var pNom = parent.getElementsByTagName("nom")[0];
                    var pFam = parent.getElementsByTagName("fam")[0];
                    var pName = parent.getElementsByTagName("name")[0];
                    var pPatr = parent.getElementsByTagName("patr")[0];
                    var pReg = parent.getElementsByTagName("reg")[0];
                    appendParent(pId.childNodes[0].nodeValue, pNom.childNodes[0].nodeValue,
                            pFam.childNodes[0].nodeValue, pName.childNodes[0].nodeValue,
                            pPatr.childNodes[0].nodeValue, pReg.childNodes[0].nodeValue);
                }
            }
        } else if (peds != null) {
            clearDialog("ped");
            if (peds.childNodes.length > 0) {
                for (loop = 0; loop < peds.childNodes.length; loop++) {
                    var ped = peds.childNodes[loop];
                    var pId = ped.getElementsByTagName("id")[0];
                    var pNom = ped.getElementsByTagName("nom")[0];
                    var pFam = ped.getElementsByTagName("fam")[0];
                    var pName = ped.getElementsByTagName("name")[0];
                    var pPatr = ped.getElementsByTagName("patr")[0];
                    var pOrg = ped.getElementsByTagName("org")[0];
                    var pDol = ped.getElementsByTagName("dol")[0];
                    var pReg = ped.getElementsByTagName("reg")[0];
                    appendPed(pId.childNodes[0].nodeValue, pNom.childNodes[0].nodeValue,
                            pFam.childNodes[0].nodeValue, pName.childNodes[0].nodeValue,
                            pPatr.childNodes[0].nodeValue, pOrg.childNodes[0].nodeValue,
                            pDol.childNodes[0].nodeValue, pReg.childNodes[0].nodeValue);
                }
            }
        } else if (dolgns != null) {
...
function validate() { // проверка данных на корректность
    ...
    if (result) {
        ...
        for (loop = 0; loop < inp.length; loop++) {
            var id = inp[loop].id;
            if ((id.substring(0, 5) == "clFam") || (id.substring(0, 4) == "clId")) {
                if (inp[loop].value != "") {
                    flag = true;
                }
            }
    ...
    if (result) {
        ...
        for (loop = 0; loop < inp.length; loop++) {
            ...
            if (val == "") {
                var id = inp[loop].id;
                if ((id.substring(0, 5) == "clFam") || (id.substring(0, 5) == "clNam") || (id.substring(0, 4) == "clDr")) {
                    result = false;
                }
            }
        }
        if (!result) {
            alert("Проверьте заполнение данных клиента");
        }
    }
