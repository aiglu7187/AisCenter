 ...
 * @author Aiglu
 */
@WebServlet(name = "SotrudViewServlet",
        loadOnStartup = 1,
        urlPatterns = {"/sotrudview"})

public class SotrudViewServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            session.removeAttribute("sotrudDolgn");
        } catch (Exception ex) {
        }
        try {
            session.removeAttribute("dolgn");
        } catch (Exception ex) {
        }
        ...
        List<SotrudDolgn> sotrudDolgn = new ArrayList<>();
        if (sotrud != null) {
            sotrudDolgn = sotrudDolgnFacade.findBySotrudAct(sotrud);
        }

        session.setAttribute("sotrudDolgn", sotrudDolgn);

        List<SprDolgn> dolgn = sprDolgnFacade.findAllDolgn();
        session.setAttribute("dolgn", dolgn);

        String url = "";
        String userPath = "";
        userPath = "/admin/sotrudview";
