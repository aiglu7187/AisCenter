...
// сервлет для перехода на станицу со списком сотрудников
@WebServlet(name = "SotrudServlet",
            loadOnStartup = 1,
            urlPatterns = "/sotrud")

public class SotrudServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            session.removeAttribute("dolgnList"); 
        }
        catch(Exception ex){}
        List<SprDolgn> dolgnList = sprDolgnFacade.findAllDolgn();
        session.setAttribute("dolgnList", dolgnList);        
        
        String url="";
        String userPath=request.getServletPath();
        if (userPath.equals("/sotrud")){
                    userPath = "/admin/sotrud";
