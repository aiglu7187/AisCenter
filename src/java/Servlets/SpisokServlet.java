 ...
 * @author Aiglu
 */
// сервлет для перехода на страницы списков клиентов
@WebServlet(name = "SpisokServlet",
            loadOnStartup = 1,
            urlPatterns = "/spisok") 
...
public class SpisokServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String idKat = request.getParameter("id");
        if (idKat.equals("children")){
            session.setAttribute("kat", "children");
            session.setAttribute("katru", "Дети");
        }
        else if (idKat.equals("parents")){
            
            session.setAttribute("kat", "parents");
            session.setAttribute("katru", "Законные представители");
        }
        else if (idKat.equals("ped")){
            
            session.setAttribute("kat", "ped");
            session.setAttribute("katru", "Педагоги");
        }
        
        String userPath = "/pup/spisokclient";
